...
const VerifyNewEmailPage = () => {
  ...
  const verifyEmailToken = useCallback(async () => {
    ...
    try {
      ...
      setStatus("error");
      const msg = error.response?.data?.error || "Failed to verify email. Please try again.";
      setMessage(msg);
      toast.error(msg);
    }
  }, [token]);
        ...
        {status === "error" && (
          <div className="space-y-2">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-blue-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transform hover:scale-[1.02] transition-all disabled:opacity-60"
            >
