...
const OrderStatusProgress: React.FC<OrderStatusProgressProps> = ({ currentStatus }) => {
  const currentIndex = statuses.findIndex(
    (status) => status.key.toLowerCase() === currentStatus.toLowerCase()
  );
  ...
  const getStatusStyle = (index: number) => {
    ...
    if (isActive) {
      return {
        circle: `bg-gradient-to-r ${status.color} border-4 border-white shadow-lg scale-105`,
        text: "text-slate-900",
    ...
    return {
      circle: "bg-slate-200 border-4 border-white shadow-md",
      text: "text-slate-400",
